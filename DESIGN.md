...
- States: default, active loading (aria-busy plus existing spinner), pending validation with explicit recovery actions, success/fresh omission, empty, stale cached data, and failed refresh with actionable copy. After three retry attempts, the panel keeps the concise pending-validation state without repeating a degraded-error explanation. Other Drops appears only for the unfiltered All view with an empty search, preserves Twitch progress, never offers a synthetic Add action, and provides an Open Twitch Drops action.
