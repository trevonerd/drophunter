...
- Queue ownership: a favorite is category-level, while each queued campaign keeps its own identity. Automatic favorites follow expiry order; manually added campaigns keep their relative order and require Start. Separate campaigns for the same favorite category are not duplicates. Removing a star preserves manual entries and the current campaign.
...
- Variants: on, off, and enabled with a transient event. Auto-start is independent of notification permissions. No event is shown while automation is off or the Twitch session is unavailable.
...
- States: idle names the selected campaign or asks for a selection; running presents the active reward exactly once through `CompactDropCard`; paused and operational farming recovery preserve the compact progress readout while explicitly saying progress is not advancing. Personal Twitch playback, including background tabs, suppresses automated transport until all personal streams stop and the 30-second grace ends. Browser-restart recovery remains internal; terminal states explain what happens next. Popup and monitor both consume `src/shared/user-status.ts`. Recovery shows one reason plus an optional retry time; attempt counters, repeated queue-preservation copy, background Twitch-session retries, and ordinary watch-health details do not belong here.
...
- Auto-start consequence: when enabled, concise text by session actions explains that a favorite can restart after Stop/Pause; disabling the auto-start switch keeps farming stopped. Use the existing muted text and polite status primitives, without adding a modal.
...
- Variants: no selection, selected campaign, selector hidden while running, onboarding pulse, selected-not-in-queue first row, draggable/reorderable future manual queue, running queued list that excludes and protects the current campaign, clear confirmation. Favorite priority remains expiry-based; manual reordering does not change that policy.
...
- States: default, active loading (aria-busy plus existing spinner), pending validation, success/fresh omission, empty, stale cached data, and failed refresh with actionable copy. After three retry attempts, the panel keeps concise pending-validation feedback without repeating a degraded-error explanation. Other Drops appears only for the unfiltered All view with an empty search, preserves Twitch progress, never offers a synthetic Add action, and provides an Open Twitch Drops action.
...
- Variants: RUNNING, PAUSED, RECOVERING, STOPPED, IDLE, COMPLETE, ATTENTION, MANUAL, and SYNCING; nearest reward vs no pending reward; active streamer vs no active streamer. Badge and context copy come from `src/shared/user-status.ts`, so monitor and popup never reinterpret the same runtime state differently.
