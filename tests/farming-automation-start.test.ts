...
describe('Farming automation start', () => {
  test('starts the highest-ranked eligible campaign through the public interface', async () => {
    ...
    expect({
      outcome,
      selected: gameKey(fixture.state.appState.selectedGame ?? fixture.best),
      events: fixture.events,
    }).toEqual({
      outcome: { kind: 'started', campaignKey: gameKey(fixture.best), transition: 'start' },
      selected: gameKey(fixture.best),
      events: [
        'refresh',
        'commit',
        'facts',
  ...
  test('returns every cheap unchanged gate before Twitch refresh', async () => {
    ...
    const outcomes = await Promise.all([
      disabled.automation.request('periodic'),
      paused.automation.request('periodic'),
      empty.automation.request('periodic'),
    ]);

    // Then: gate reasons are stable and only candidate discovery needs Twitch refresh.
    expect({
      outcomes,
      refreshes: [disabled.refreshCount(), paused.refreshCount(), empty.refreshCount()],
    }).toEqual({
      outcomes: [
