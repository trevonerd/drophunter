...
describe('runtime message protocol', () => {
  ...
  test('accepts payload-free popup activation commands and rejects payloads', () => {
    expect(isRuntimeRequest({ type: 'ACTIVATE_POPUP' })).toBe(true);
    expect(isRuntimeRequest({ type: 'OPEN_DROPS_AND_SYNC' })).toBe(true);
    expect(isRuntimeRequest({ type: 'ACTIVATE_POPUP', payload: {} })).toBe(false);
    expect(isRuntimeRequest({ type: 'OPEN_DROPS_AND_SYNC', payload: { active: false } })).toBe(false);
  });
