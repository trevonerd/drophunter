...
describe('AFK Twitch authentication recovery', () => {
  ...
  test('keeps a valid cached session during Hidden polling with no Twitch tabs open', async () => {
    ...
    globalThis.fetch = async () => {
      fetchCalls += 1;
      return new Response(JSON.stringify([{ data: { user: { id: 'channel-1', stream: null } } }]), {
        status: 200,
  ...
  test('keeps farming silently after the first claimed drop when the Drops page is closed', async () => {
    const state = createState();
    const cachedSession = { ...twitchSession, clientIntegrity: 'existing-integrity-token' };
    state.twitchSessionCache = cachedSession;
    let recoveryRequests = 0;
    let createdTabs = 0;
    chromeMocks.chrome.tabs.create = async (properties) => {
      createdTabs += 1;
    ...
    await checkDropProgress(state, {
      ...
      onAcquireStreamerForSelectedGame: async () => true,
    ...
    expect(recoveryRequests).toBe(0);
    expect(createdTabs).toBe(0);
    expect(state.twitchSessionCache).toEqual(cachedSession);
    expect(state.appState.recoveryReason).toBeNull();
